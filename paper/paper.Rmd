...
abstract: |
  Bulk tumor transcriptomes reflect a mixture of malignant cells and microenvironmental components, and the gene programs that explain the most expression variance need not be the programs most associated with patient outcomes. Standard nonnegative matrix factorization (NMF) discovers latent structure by minimizing reconstruction error, which may be dominated by variance-dominant but prognostically neutral signals such as tissue composition. Here we present DeSurv, a survival-supervised deconvolution framework that jointly integrates NMF with Cox proportional hazards modeling, directing gene programs toward outcome-relevant structure during discovery rather than only in post hoc evaluation. In simulations with known latent structure, DeSurv more reliably recovers the true factorization rank and the identity of prognostic gene programs than standard NMF, with the largest advantage when variance and prognosis diverge. In pancreatic ductal adenocarcinoma (PDAC), DeSurv concentrates survival signal into a parsimonious set of factors aligned with known tumor and microenvironmental programs while suppressing variance-dominant exocrine-compositional signals. These survival-aligned factors generalize across independent PDAC cohorts with consistent hazard ratios and clearer survival separation than their unsupervised counterparts. These results support survival supervision as a principled approach for discovering interpretable, outcome-aligned transcriptional programs in heterogeneous tumors.
