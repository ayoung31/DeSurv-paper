...
the alignment between expression variance and prognostic relevance. The fraction
of expression variance explained by factor $j$ was computed as:
$$V_j = \frac{\| W_{\cdot j}\, H_{j \cdot} \|_F^2}{\| X \|_F^2}$$
where $W_{\cdot j}$ is the $j$-th column of $W$ and $H_{j \cdot}$ is the $j$-th
row of $H$, so $W_{\cdot j} H_{j \cdot}$ is the rank-1 approximation contributed
by factor $j$.
