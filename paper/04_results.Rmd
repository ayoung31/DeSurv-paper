...
lower = plot_grid(fig_bo_heat_tcgacptac,
                  alt_plots$k_hist,ncol=2,labels=c("D","E"),rel_widths = c(3,2))
...
# Both KM panels without inline legend (equal plot heights)
mid   <- stack_surv(med_desurv, keep_legend = FALSE)
right <- stack_surv(med_std,    keep_legend = FALSE)

# Shared strata legend spanning both B and C
shared_legend <- get_legend(
  med_desurv$plot +
    theme_pnas +
    theme(legend.position = "bottom") +
    guides(color = guide_legend(nrow = 1, title = "Strata"))
)
...
# Add shared legend below B+C
bc_with_legend <- plot_grid(bc, shared_legend, ncol = 1, rel_heights = c(12, 1))
