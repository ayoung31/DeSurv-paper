...
Within this framework, the product ($X^TW$) represents patient-level factor scores - the inferred burden of each latent program across subjects. These factor scores serve as covariates in the Cox model, and their regression coefficients ($\beta$) indicate whether higher activity of a given program corresponds to improved or reduced survival. 
...
Finally, to assess disease-specific structure, we fit DeSurv directly to the bladder cancer cohort and examined the biological programs associated with the resulting factors (Fig. \ref{fig:bladder}C). The survival-associated DeSurv factor identified in bladder cancer overlapped with established bladder tumor programs linked to aggressive disease, while variance-dominant NMF factors were enriched for transcriptional structure lacking consistent association with outcome. As in PDAC, differentiation- and composition-related signals were prominent among NMF components but were not isolated as dominant axes in the survival-informed DeSurv solution.

Together, these results demonstrate that survival-informed factorization both transfers prognostic structure across cancer types and adapts to disease-specific biology when refit within a cohort. By deprioritizing variance-dominant but prognostically neutral signals and concentrating outcome-relevant information into a small number of factors, DeSurv identifies latent structure that is robust across cancers while remaining biologically interpretable within each disease context.

```{r fig-bladder, fig.cap="Survival-informed factorization transfers and refits in bladder cancer. (A) Relationship between expression variance explained and survival contribution for factors derived from standard nonnegative matrix factorization (NMF) in bladder cancer. Factors explain substantial fractions of transcriptional variance but show minimal association with survival, indicating decoupling between variance dominance and prognostic relevance. (B) Kaplan–Meier analysis in bladder cancer based on projection of the DeSurv model trained in pancreatic ductal adenocarcinoma (PDAC). Median stratification of the projected survival-aligned factor separates patients by outcome, demonstrating transfer of prognostic structure across cancer types. (C) Biological interpretation of factors obtained by fitting DeSurv directly to bladder cancer data. The survival-associated factor overlaps with established bladder tumor programs linked to aggressive disease, whereas variance-dominant NMF factors lack consistent association with outcome. \\label{fig:bladder}", fig.env='figure*', fig.pos='t', out.width='\\textwidth'}
