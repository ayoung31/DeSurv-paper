...
repo_root <- if (file.exists("_targets.R")) "." else ".."

get_targets_store <- function(root) {
  candidates <- c(file.path(root, "_targets.yaml"), file.path(root, "paper", "_targets.yaml"))
  for (path in candidates) {
    if (file.exists(path)) {
      lines <- readLines(path, warn = FALSE)
      store_line <- grep("^\\s*store\\s*:", lines, value = TRUE)
      if (length(store_line)) {
        return(sub("^\\s*store\\s*:\\s*", "", store_line[1]))
      }
    }
  }
  ""
...
if (is.null(store) || !nzchar(store)) {
  store <- get_param_value("tar_store", get_targets_store(repo_root))
  if (is.null(store) || !nzchar(store)) {
...
make_panel <- function(history_df, labels, include_alpha = TRUE, cindex_label = "Best CV C-index across k") {
  ...
  if (include_alpha) {
    p_k_alpha <- ggplot(best_per_k, aes(x = k, y = alpha_best, group = 1)) +
      ...
      labs(x = "Rank k", y = "Selected alpha") +
      theme_minimal(base_size = 12) +
      theme(legend.position = "none")
...
bo_history_supervised <- prepare_bo_history(tar_read_name(resolve_target_name(
  "desurv_bo_history",
  bo_label = bo_label,
  available = available_targets
)))
bo_history_alpha0 <- prepare_bo_history(tar_read_name(resolve_target_name(
  "desurv_bo_history_alpha0",
  bo_label = bo_label,
  available = available_targets
)))

bo_results_supervised <- tar_read_name(resolve_target_name(
  "desurv_bo_results",
  bo_label = bo_label,
  available = available_targets
))
bo_results_alpha0 <- tar_read_name(resolve_target_name(
  "desurv_bo_results_alpha0",
  bo_label = bo_label,
  available = available_targets
))

fit_std <- tar_read_name(resolve_target_name(
  "fit_std",
  bo_label = bo_label,
  run_label = run_label,
  available = available_targets
...
ora_results <- tar_read_name(resolve_target_name(
  "ora_analysis_desurv",
  bo_label = bo_label,
  run_label = run_label,
  available = available_targets
...
fit_consensus <- tar_read_name(resolve_target_name(
  "tar_fit_desurv",
  bo_label = bo_label,
  run_label = run_label,
  available = available_targets
))
tops_desurv <- tar_read_name(resolve_target_name(
  "tar_tops_desurv",
  bo_label = bo_label,
  run_label = run_label,
  available = available_targets
