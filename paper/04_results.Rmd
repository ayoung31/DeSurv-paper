...
Figure \@ref(fig:fig-bo) summarises the optimisation run. Panel B tracks the improvement in validation C-index across the initial Latin-hypercube design and the subsequent BO iterations, while Panel C overlays every sampled parameter combination. The final configuration converges to $k=3$ and $\alpha=0.7$, which provided the highest cross-validated C-index encountered during the search.
...
```{r fig-bo, fig.width = 6, fig.height= 5.5, fig.cap="A. Overview of the DeSurv training and Bayesian optimisation workflow. B. Mean validation C-index achieved at each Bayesian-optimisation evaluation; the star marks the best setting. C. Parameter combinations explored by Bayesian optimisation, coloured by validation C-index. D. Conventional NMF diagnostics (cophenetic, dispersion, residuals, explained variance, silhouette, and sparseness) for the unsupervised baseline." , fig.env='figure*', fig.pos='t', out.height= "5.5in", out.width="6in"}
tar_load_globals()
