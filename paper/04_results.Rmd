...
make_cindex_violin <- function(history_df, label) {
  ...
  history_df <- history_df %>%
    mutate(k = factor(.data[[k_col]], levels = k_levels))

  ggplot(history_df, aes(x = k, y = mean_cindex)) +
    geom_violin(fill = "#a6cee3", color = NA, scale = "width", alpha = 0.8) +
    geom_boxplot(width = 0.12, outlier.size = 0.4, alpha = 0.7, fill = "#1f78b4", color = "#1f78b4") +
    labs(
      x = "Rank k",
...
extract_gp_curve <- function(bo_results, history_df) {
  ...
  best_per_k <- history_df %>%
    ...
    slice_head(n = 1) %>%
    ungroup() %>%
    mutate(k_numeric = as.numeric(.data[[k_col]])) %>%
    arrange(k_numeric)

  newdata_actual <- best_per_k %>%
...
make_gp_curve_plot <- function(curve_df, label) {
  ggplot(curve_df, aes(x = k, y = mean, group = 1)) +
    geom_ribbon(
      aes(ymin = lower, ymax = upper),
      fill = "#b2df8a",
      alpha = 0.4
    ) +
    geom_line(color = "#33a02c", linewidth = 0.7) +
    geom_point(color = "#33a02c", size = 2) +
    scale_x_continuous(breaks = curve_df$k) +
    labs(
      x = "Rank k",
...
bo_results_supervised <- tar_read(desurv_bo_results)
bo_results_alpha0 <- tar_read(desurv_bo_results_alpha0)
