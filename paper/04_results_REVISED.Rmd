...
# Unpack heatmap plots and legends from list targets
...
plot_4c <- fig_desurv_std_correlation_tcgacptac$plot

# Align 4A and 4B to identical height and width
aligned_ab <- cowplot::align_plots(plot_4a, plot_4b, align = "hv", axis = "tblr")

# Left block: 4A, 4B, then shared legend to the right
left_block <- plot_grid(
  aligned_ab[[1]], aligned_ab[[2]], legend_4ab,
  ncol = 3, labels = c("A", "B", ""),
  rel_widths = c(2.5, 2.5, 0.5)
)

right <- plot_grid(NULL, plot_4c, ncol = 2, rel_widths = c(1, 4), labels = c("", "C"))

plot_grid(left_block, right, ncol = 2, rel_widths = c(5, 3))
...
# -------------------------
stack_surv <- function(surv_obj, keep_legend = FALSE, remove_ylab = FALSE) {
  p <- surv_obj$plot +
    theme_pnas +
    theme(
      legend.position = if (keep_legend) "bottom" else "none",
      axis.title.x = element_blank(),              # x-label goes on the table panel
      axis.title.y = if (remove_ylab) element_blank() else element_text(),
...
# Shared legend spanning both panels placed below
# -------------------------
med_desurv <- fig_median_survival_desurv_tcgacptac
med_std    <- fig_median_survival_std_desurvk_tcgacptac

mid   <- stack_surv(med_desurv, keep_legend = FALSE)
right <- stack_surv(med_std,    keep_legend = FALSE, remove_ylab = TRUE)

# Shared strata legend: built from a simple dummy ggplot for reliability
leg_df <- data.frame(
  x = 1:2, y = 1:2,
  Strata = factor(c("Low (< median)", "High (\u2265 median)"),
                  levels = c("Low (< median)", "High (\u2265 median)"))
)
shared_legend <- cowplot::get_legend(
  ggplot2::ggplot(leg_df, ggplot2::aes(x = x, y = y, color = Strata)) +
    ggplot2::geom_line(linewidth = 1) +
    ggplot2::scale_color_manual(
      values = c("Low (< median)" = "violetred2", "High (\u2265 median)" = "turquoise4"),
      name = "Strata"
    ) +
    theme_pnas +
...
bc_with_legend <- plot_grid(bc, shared_legend, ncol = 1, rel_heights = c(10, 1))
