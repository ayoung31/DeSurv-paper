...
We developed DeSurv, a survival-supervised deconvolution framework that jointly optimizes NMF reconstruction and Cox proportional hazards likelihood to learn gene programs shaped by both transcriptomic structure and patient outcomes (Fig. \ref{fig:schema}). A supervision parameter $\alpha$ controls the balance between reconstruction fidelity and survival prediction; when $\alpha = 0$, DeSurv reduces to standard unsupervised NMF. The survival gradient acts on the gene program matrix $W$ and the Cox coefficients $\beta$ but not on the sample loadings $H$, directing gene programs toward expression patterns predictive of patient outcomes while preserving $H$'s interpretation as cell-type mixture proportions. Under a fixed factorization rank, increasing $\alpha$ trades reconstruction fidelity for improved capture of outcome-relevant structure. Because $W$ defines shared transcriptomic programs, new samples can be scored by projection ($Z_{\text{new}} = W^\top X_{\text{new}}$) without requiring survival data; methods that instead route supervision through $H$ learn parameters tied to training individuals, limiting direct transferability. Factor-specific signature genes are identified from each column of $W$ as genes with high weights in that column relative to all others, following standard NMF practice [@brunet2004metagenes; @moffitt2015virtual]; these gene lists are used in subsequent analyses to characterize factor biology and compare programs across methods and cohorts. Model selection, including rank $k$ and $\alpha$, is performed via cross-validated concordance (Methods). The complete model formulation, notation, and optimization procedure are provided in the Methods and SI Appendix.
...
**Standard rank selection metrics disagree; survival-driven optimization provides a clear, principled choice.**
...
**DeSurv outperforms standard NMF when prognostic signal exists and defaults to standard NMF when no signal is present.**
...
**DeSurv-derived factors maintain prognostic associations in independent PDAC cohorts, while variance-driven factors show greater heterogeneity.**
