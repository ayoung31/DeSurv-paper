...
compartment-specific deconvolution across 33 cancer types (5).
...
replicates, DeSurv achieved median C-index \(0.837\) versus \(0.725\)
for standard NMF (\(\Delta = 0.112\); \(p = 3{,}000\) genes, \(n = 200\)
samples, true \(k = 3\)).
...
well-established prognostic role of basal-like biology in PDAC (10,
\textbf{moffit2015virtual?}) --- reflects two features of the
factorization: D3 absorbs residual exocrine-compositional variation
alongside basal-like programs (see factor correspondence below),
diluting its prognostic specificity; and the survival-relevant contrast
between classical and basal-like biology is already captured by D1's
iCAF-classical co-program, leaving D3's conditional contribution
minimal. This result directly illustrates the misalignment between
variance and prognosis anticipated by sufficient dimension reduction
theory (9) and observed empirically in tumor purity analyses (12): the
factors that explain the most transcriptomic variation need not be the
factors most associated with patient outcomes. The observed pattern in
PDAC is consistent with the intermediate regime between the primary and
mixed simulation scenarios, where DeSurv's advantage was largest.
...
transcriptional background (\(n = 51\), \(34\) events; log-rank
\(P = 0.68\); HR \(0.66\); 95\% CI \(0.09\)--\(4.81\)). This finding
suggests that survival supervision captures cross-cancer biology that
unsupervised methods may distribute across multiple outcome-neutral
factors.
...
supervision isolates a generalizable prognostic program. For the
dominant DeSurv program, prognostic signal transferred to bladder
cancer, supporting the hypothesis that outcome-guided learning captures
biology that generalizes more readily than variance-driven structure.
...
prognostic signal that supervision would have concentrated. A
K\(\times\alpha\) sensitivity analysis confirmed this parsimonious
selection: with production-optimal gene focusing (ntop\(=270\)), K\(=3\)
validated adjustedly across a broad \(\alpha\) range, while the apparent
advantage of K\(=7\) observed under all-gene projection disappeared
entirely, demonstrating that the one-standard-error rule correctly
identified K\(=3\) as the stable, generalizable solution. Our
...
## Paper git commit: a5db7f9666c208c97c4d2fc3abd0ac4b9a24144a
\end{verbatim}
