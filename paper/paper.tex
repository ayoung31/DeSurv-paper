...
optimization selected \(k = 3\) and \(\alpha = 0.3339356\) for DeSurv in
the PDAC training cohorts (TCGA and CPTAC). To enable a direct
comparison of how each method organizes the same number of factors, we
also fit standard NMF at the same rank (\(k = 3\)); results for standard
NMF at independently selected ranks (\(k = 5\) via elbow detection,
\(k = 7\) via cross-validated concordance at \(\alpha = 0\)) are
presented in the SI Appendix. We note that standard NMF at \(k = 5\)
achieves comparable external validation performance to DeSurv at
\(k = 3\); the DeSurv advantage lies in parsimony --- recovering
equivalent prognostic generalization with fewer, more interpretable
factors --- rather than in raw concordance at each method's unrestricted
optimum.

DeSurv produced a factorization that concentrated survival signal into a
single factor (D1) integrating classical tumor programs with a restCAF
(iCAF-like) stromal signature (Fig. \ref{fig:bio}A). This mirrors
...
tumors in a restCAF/iCAF microenvironment demonstrating the most
favorable outcomes (11). A second factor (D2) captured an activated
...
in Cox model log partial likelihood in the training data. Across 5
independent PDAC cohorts (\(n = 616\)), the DeSurv-derived factor
exhibited consistent survival effects, with hazard ratio estimates
showing limited variability and predominantly protective associations
(pooled HR \(1.78\); 95\% CI \(1.48\)--\(2.13\);
\(P = \ensuremath{4.4\times 10^{-10}}\); SI Appendix, Fig. S9). In
contrast, the NMF factor identified by the same criterion showed greater
heterogeneity across datasets and weaker survival associations,
consistent with the expectation that variance-driven programs capture
cohort-specific variation that does not transfer.
...
clear separation of survival trajectories was observed (Fig.
\ref{fig:bio}C). The same procedure applied to NMF yielded weaker
survival stratification (Fig. \ref{fig:bio}D). These results support the
prediction that outcome-aligned programs capture biology that
...
DeSurv would have selected \(k = 7\) at \(\alpha = 0.3339356\)---the
same rank independently favored by standard NMF under cross-validated
...
Bayesian optimization selected \(k = 3\) and \(\alpha = 0.3339356\) as
the primary model configuration, but whether this reflects a stable
solution or a fragile local optimum required further evaluation. To
assess robustness, we conducted an exhaustive sensitivity analysis
...
its survival association in 616 independent validation patients across 4
external cohorts, using stratified Cox regression with and without
adjustment for PurIST and DeCAF classifiers.
...
microdissection (4), experimental microdissection (23), and unsupervised
...
(24) both predict that supervised compression retains outcome-relevant
...
## Paper git commit: 942f14db88bd8ac8179f0f3bd1540910dbb66ed1
\end{verbatim}
