...
proposed between two and six subtypes (13, 14), ultimately converging on
a robust basal/classical dichotomy only after extensive retrospective
evaluation across independent cohorts (1, 7, 10).
...
single factor (D1) combining classical tumor programs with an activated
iCAF microenvironmental signature (Fig. \ref{fig:bio}A) --- consistent
with the known co-occurrence of classical tumor identity and
iCAF-enriched stroma in PDAC {[}REF{]} and with evidence that iCAF
activation is independently associated with patient outcomes {[}REF{]}.
A second factor (D2) captured an activated immune-stromal
...
conditional analysis, reflecting both the absorption of residual
exocrine-compositional variation and the concentration of the
...
contribution (SI Appendix, Fig. \ref{fig:varsurvival}). A striking
pattern emerged: all three NMF factors contributed negligibly to
survival despite spanning a range of conditional variance, while DeSurv
concentrated nearly all survival signal into D1. Specifically, the
DeSurv factor with the largest survival contribution explained \(7\)\%
of variance with \(\Delta\ell = 66.4\), while the highest-variance NMF
factor explained \(23.9\)\% of expression variance but contributed only
...
well-established prognostic role of basal-like biology in PDAC {[}REF{]}
...
Appendix, Fig. \ref{fig:corr}). NMF's classical-associated factor (N1)
showed the strongest correspondence with DeSurv's basal-like factor
(D3), and NMF's composite microenvironmental factor (N3) mapped
primarily to DeSurv's microenvironmental factor (D2). The exocrine-like
NMF factor (N2) did not correspond strongly to any single DeSurv factor;
its signal was weakly absorbed into D3, consistent with D3's high
conditional variance and low survival contribution described above.
Together, these results demonstrate that DeSurv selectively preserves
biologically distinct tumor and microenvironmental structure while
reorganizing variance-dominant but prognostically inert variation ---
including exocrine-compositional signal --- into a high-variance,
low-survival factor rather than allocating dedicated model capacity to
it.
...
\(P = \ensuremath{4.4\times 10^{-10}}\); Supplementary Fig.
\ref{fig:forest}). In contrast, the NMF factor identified by the same
...
transcriptional background (\(n = 260\), \(172\) events; log-rank
\(P = 0.36\); HR \(1.17\); 95\% CI \(0.84\)--\(1.64\)). This finding
...
## Paper git commit: 421e0e5e240d3ff3334d280af04cec4726721e53
\end{verbatim}
