...
parsimonious choice. Bayesian optimization selected
\(k = [[PNAS_TODO:k_selected from tar_k_selection_tcgacptac]]\) and
\(\alpha = [[PNAS_TODO:alpha from tar_params_best_tcgacptac]]\) (C-index
\([[PNAS_TODO:best_mean from tar_k_selection_tcgacptac]]\); 1-SE rule;
\(n = [[PNAS_TODO:ncol(tar_data_filtered_tcgacptac\)ex){]}{]}\$
patients,
\([[PNAS_TODO:sum(tar_data_filtered_tcgacptac\)sampInfo\(event)]]\)
events).
...
replicates, DeSurv achieved median C-index
\([[PNAS_TODO:median_cindex_desurv from sim_figs_by_scenario R0_easy]]\)
versus \([[PNAS_TODO:median_cindex_nmf]]\) for standard NMF
(\(\Delta = [[PNAS_TODO:delta_cindex]]\); \(p = 3{,}000\) genes,
\(n = 200\) samples, true \(k = 3\)).
...
the highest-variance NMF factor explained
\([[PNAS_TODO:max_varexpl_nmf]]\)\% of expression variance but
contributed \(\Delta\ell = [[PNAS_TODO:delta_loglik_nmf_maxvar]]\) to
survival, whereas the DeSurv factor with the largest survival
contribution explained \([[PNAS_TODO:varexpl_desurv_maxsurv]]\)\% of
variance with
\(\Delta\ell = [[PNAS_TODO:delta_loglik_desurv_maxsurv]]\). The
remaining DeSurv factors contributed minimal survival signal, indicating
that survival-relevant information was not diffusely distributed across
factors. This result directly illustrates the misalignment between
...
PDAC, where the highest-variance factor shows minimal survival
association while other factors show partial overlap between variance
and prognosis, is consistent with the intermediate regime between the
...
independent PDAC cohorts (\(n = [[PNAS_TODO:total_val_n]]\)), the
...
associations (pooled HR \([[PNAS_TODO:pooled_hr_desurv]]\); 95\% CI
\([[PNAS_TODO:ci_lower]]\)--\([[PNAS_TODO:ci_upper]]\); log-rank
\(P = [[PNAS_TODO:logrank_p]]\); Fig. \ref{fig:extval}A). In contrast,
...
transcriptional background (\(n = [[PNAS_TODO:n_bladder]]\),
\([[PNAS_TODO:events_bladder]]\) events; log-rank
\(P = [[PNAS_TODO:logrank_p_bladder]]\); HR
\([[PNAS_TODO:hr_bladder]]\); 95\% CI \([[PNAS_TODO:ci_bladder]]\)).
This finding suggests that survival supervision captures cross-cancer
biology that unsupervised methods may distribute across multiple
outcome-neutral factors.
...
## Paper git commit: 6c60bd7360b8a4b8ab16cffb257a1f98477156d1
\end{verbatim}
