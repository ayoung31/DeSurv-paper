...
most expression variance need not be the programs most associated with
...
with Cox proportional hazards modeling, directing gene programs toward
...
(PDAC), DeSurv concentrates survival signal into a parsimonious set of
...
unsupervised counterparts. These results support survival supervision as
a principled approach for discovering interpretable, outcome-aligned
transcriptional programs in heterogeneous tumors.
\end{abstract}
...
clinical outcomes (1). Bulk RNA-sequencing remains the primary source of
the large, clinically annotated cohorts required for such survival
modeling, but each expression profile is a composite of malignant cells,
cancer-associated fibroblasts, immune infiltrates, and other
microenvironmental components (2). As a result, these signatures may be
diluted or obscured by variation in cellular composition across samples.
...
(3--5). This approach has yielded important biological insights,
including the identification of basal-like and classical tumor programs
in pancreatic ductal adenocarcinoma (PDAC) (1, 4) and
compartment-specific deconvolution in pancreatic cancer (5).
...
biology (8, 9). In PDAC, exocrine and tissue-composition signals reflect
low tumor purity rather than cancer cell-intrinsic programs and carry
little prognostic value (10, 11). Although less prevalent in bulk
tissue, the tumor-intrinsic basal-like and activated stromal programs
carry the strongest survival associations (4), yet risk being diluted or
missed entirely when decomposition prioritizes explaining maximal
variance in expression. This misalignment between variance and prognosis
extends beyond PDAC: tumor purity alone accounts for a large fraction of
expression variation across cancer types (12), yet the objective
optimized during unsupervised discovery (reconstruction error) differs
fundamentally from the criterion used during evaluation (survival
association) (9). Identifying the prognostically relevant subset then
requires extensive downstream filtering that is ad hoc, cohort-specific,
and difficult to reproduce (13). Over a decade of PDAC subtyping efforts
proposed between two and six subtypes (1, 4, 13, 14), ultimately
converging on a robust basal/classical dichotomy only after extensive
retrospective evaluation across independent cohorts (10).
...
We evaluate DeSurv in two settings that test the predictions above.
...
(3, 17). We evaluated DeSurv in two settings: controlled simulations and
...
unsupervised rank selection heuristics (6, 20). Using gene expression
data from the PDAC training cohorts (TCGA (21) and CPTAC (22); Methods),
...
\(\alpha = 0.334\) (C-index \(0.655\); 1-SE rule; \(n = 273\) patients,
\(139\) events).
...
concentrating near \(k = 2\). These results support the prediction that
incorporating outcome information into model selection improves recovery
of the true factorization rank.
...
for standard NMF (\(\Delta = 0.115\); \(p = 3{,}000\) genes, \(n = 200\)
samples, true \(k = 3\)).
...
In PDAC, standard NMF distributes its factors across variance-dominant
but prognostically inert programs; DeSurv reorganizes the learned factor
structure to concentrate survival signal into a single classical-iCAF
axis.
...
SI Appendix. We note that standard NMF at \(k = 5\) achieves comparable
external validation performance to DeSurv at \(k = 3\); the DeSurv
advantage lies in parsimony --- recovering equivalent prognostic
generalization with fewer, more interpretable factors --- rather than in
raw concordance at each method's unrestricted optimum.

DeSurv produced a factorization that concentrated survival signal into a
single factor (D1) integrating classical tumor programs with an
iCAF-associated stromal signature (Fig. \ref{fig:bio}A). This mirrors
observations that survival stratification in PDAC is strongest when
tumor-intrinsic and CAF subtypes are considered jointly, with classical
tumors in an iCAF microenvironment demonstrating the most favorable
outcomes (11, 23). A second factor (D2) captured an activated
immune-stromal microenvironment. A third factor (D3) was enriched for
basal-like tumor programs but, as discussed below, contributed minimally
to survival in conditional analysis, reflecting both the absorption of
residual exocrine-compositional variation and the concentration of the
prognostically relevant tumor-stroma contrast within D1. Notably, no
...
Bailey ADEX; Fig. \ref{fig:bio}A), suggesting that survival supervision
deprioritizes differentiation-related variation that is weakly
...
biology (Fig. \ref{fig:bio}B). One factor was strongly and significantly
correlated with exocrine-associated gene programs (DECODER Exocrine,
Collisson Exocrine, Bailey ADEX; Fig. \ref{fig:bio}B) and negatively
correlated with immune and stromal signatures, consistent with a bulk
composition axis separating normal or differentiated tissue from
non-epithelial tumor content. A second factor aligned with classical
tumor identity, while a third aggregated immune, fibroblast, and
extracellular matrix-related programs --- including Puleo Desmoplastic,
SCISSORS panCAF and myCAF, Maurer Immune and ECM, and DECODER Activated
Stroma --- into a single composite microenvironmental factor (Fig.
\ref{fig:bio}B). These patterns indicate that, given the same number of
factors, standard NMF allocates substantial model capacity to
differentiation-driven signals and merges distinct microenvironmental
states that DeSurv separates.
...
factor (D3, \textasciitilde30\% of conditional variance) also
contributed minimally to survival. This apparent paradox --- given the
well-established prognostic role of basal-like biology in PDAC (4, 10)
--- reflects two features of the factorization: D3 absorbs residual
exocrine-compositional variation alongside basal-like programs (see
factor correspondence below), diluting its prognostic specificity; and
the survival-relevant contrast between classical and basal-like biology
is already captured by D1's iCAF-classical co-program, leaving D3's
conditional contribution minimal. This result directly illustrates the
misalignment between variance and prognosis anticipated by sufficient
dimension reduction theory (9) and observed empirically in tumor purity
analyses (12): the factors that explain the most transcriptomic
variation need not be the factors most associated with patient outcomes.
The observed pattern in PDAC is consistent with the intermediate regime
between the primary and mixed simulation scenarios, where DeSurv's
advantage was largest.
...
For each method (both fit at \(k = 3\), the DeSurv BO-selected rank;
external validation C-index for NMF at independently selected ranks is
summarized in SI Appendix, Table S4), we focused on the factor showing
the largest increase in Cox model log partial likelihood in the training
data. Across 5 independent PDAC cohorts (\(n = 616\)), the
DeSurv-derived factor exhibited consistent survival effects, with hazard
ratio estimates showing limited variability and predominantly protective
...
When validation samples were pooled and stratified using the
cross-validated optimal cutpoint applied to the DeSurv linear predictor,
clear separation of survival trajectories was observed (dichotomized
high/low groups, Fig. \ref{fig:bio}C). The same procedure applied to NMF
yielded weaker survival stratification (Fig. \ref{fig:bio}D). These
results support the prediction that outcome-aligned programs capture
biology that generalizes, whereas variance-driven programs may include
...
production-optimal values (\(\lambda = 0.349\), \(\nu = 0.056\),
ntop\(= 270\), 100 initializations). For each fit, we identified the
factor most correlated with the production K\(=3\) Factor 1 and assessed
its survival association in 570 independent validation patients across 4
external cohorts (PACA-AU array and RNA-seq are treated as a single
cohort here; the generalization analysis above counts them separately,
yielding \(n = 616\) across 5 datasets), using stratified Cox regression
with and without adjustment for PurIST and DeCAF classifiers.

With the production-optimal gene-projection setting (ntop\(=270\)),
K\(=3\) validated adjustedly at 5 of 7 supervision-strength values,
including \(\alpha = 0.55\) where it achieved adjusted HR \(= 0.81\)
(\(p = 0.003\)) --- nearly identical to the production fit
(\(p = 0.004\)). K\(=5\) at \(\alpha = 0.55\) provided independent
corroboration at higher factor-program fidelity (Spearman H-score
correlation = 0.903 vs.~K\(=3\)'s 0.855; adjusted \(p = 0.019\)),
confirming the iCAF biology is recoverable by a higher-rank model. In
contrast, K\(=7\) --- which had appeared to achieve adjusted
significance (\(p = 0.019\)) in a parallel analysis using all 1970 genes
in the projection --- completely failed with ntop\(=270\) (adjusted
\(p = 0.676\)), establishing that the earlier K\(=7\) advantage was an
artifact of the all-genes projection rather than an intrinsic property
of the 7-factor solution.

To isolate the contribution of gene focusing, we repeated the analysis
with all genes retained (ntop\(=\text{NULL}\)) while keeping \(\lambda\)
and \(\nu\) at their optimal values. With all genes, K\(=3\) at
\(\alpha = 0.55\) reached only adjusted \(p = 0.097\), failing to
reproduce the production result, and only 5 of 35 combinations achieved
adjusted \(p < 0.05\) --- compared with 18 of 35 with ntop\(=270\).
Substituting \(\lambda = 0.349\) for \(\lambda = 0.300\) within the
all-genes setting shifted the pattern of validated combinations but left
the overall count unchanged (5/35), confirming that regularization
tuning alone cannot reproduce the adjusted signal without gene focusing.
These analyses establish ntop\(=270\) as the critical hyperparameter:
focusing the projection onto the 270 most factor-characteristic genes
concentrates the survival signal, suppresses noise from the remaining
genes, and prevents overfitting at high supervision weights. Full K
\(\times\) \(\alpha\) adjusted p-value tables are provided in the
Supplementary (Tables S1--S3). Taken together, these results provide
empirical justification for the 1-SE rule's parsimony preference:
\(k = 7\), which achieved the highest training CV C-index, completely
failed adjusted external validation (\(p = 0.676\)), while \(k = 3\)
validated robustly (\(p = 0.003\)). The simpler model not only passed
the 1-SE threshold---it generalized better.
...
exocrine-compositional, classical tumor, and microenvironmental
variation --- leaving no single factor to concentrate survival
association; survival supervision corrects this allocation, enabling
recovery of basal-like and microenvironmental programs even at
\(k = 3\). In simulations, DeSurv recovered the true factorization rank
and the identity of prognostic programs more reliably than standard NMF,
with the largest advantage occurring when prognostic programs explained
modest variance relative to outcome-neutral signals. In PDAC, DeSurv
isolated tumor-intrinsic and microenvironmental programs with clear
survival associations, and these factors generalized across independent
PDAC cohorts. The K\(=3\) selection proved robust rather than fragile:
an exhaustive sensitivity analysis spanning five ranks and seven
supervision strengths, with all hyperparameters fixed at their
BO-selected values, confirmed that K\(=3\) validates adjustedly across a
wide supervision range (5 of 7 \(\alpha\) values tested; adjusted
\(p = 0.003\) at \(\alpha = 0.55\)). This robustness depended critically
on the BO-selected gene-focusing parameter (ntop\(=270\)): retaining all
genes with the same regularization reduced the number of significantly
validated K\(\times\alpha\) combinations from 18 to 5 of 35 and
eliminated K\(=3\) adjusted significance at the production supervision
strength, establishing gene focusing as the mechanism by which survival
supervision isolates a generalizable prognostic program. In PDAC, DeSurv
recapitulated known tumor and microenvironmental programs, including the
basal-like/classical distinction and activated versus normal stromal
states. The methodological contribution lies not in the identity of
these programs---which have been established through virtual
microdissection (4), experimental microdissection (24), and unsupervised
...
13), but are identified through a single optimization rather than
iterative retrospective evaluation. The semi-supervised design of DeSurv
reflects a deliberate balance between biological completeness and
...
supervision enables parsimonious factorizations by concentrating
prognostic signal into fewer factors, so that the one-standard-error
rule applied to cross-validated concordance selects smaller ranks than
would be chosen by unsupervised criteria or by optimizing concordance
without supervision. In PDAC, the cross-validated concordance surface
was relatively flat across \(k = 3\)--12 for DeSurv, indicating that
three supervised factors captured nearly as much prognostic information
as twelve; for standard NMF, concordance increased steadily from
\(k = 2\) through \(k = 7\)--11, reflecting the need for additional
factors to distribute prognostic signal that supervision would have
concentrated. A K\(\times\alpha\) sensitivity analysis confirmed this
parsimonious selection: with production-optimal gene focusing
(ntop\(=270\)), K\(=3\) validated adjustedly across a broad \(\alpha\)
range, while the apparent advantage of K\(=7\) observed under all-gene
projection disappeared entirely, demonstrating that the
one-standard-error rule correctly identified K\(=3\) as the stable,
generalizable solution. Our simulations provide further guidance on when
this tradeoff is most beneficial: DeSurv's advantage is greatest when
prognostic programs explain modest variance relative to outcome-neutral
signals and vanishes under null conditions where no survival signal
exists. DeSurv is therefore not intended to replace unsupervised NMF in
all settings. When the goal is exploratory discovery without clinical
endpoints, when survival data is sparse or unreliable, or when the
application requires comprehensive biological characterization rather
than prognostic stratification, unsupervised methods remain appropriate.
The \(\alpha\) parameter, tuned via Bayesian optimization, lets the data
determine the appropriate balance for a given cancer type and cohort
size, and the \(\alpha = 0\) endpoint recovers standard NMF as a special
case.
...
projection. Third, DeSurv was applied here only to PDAC, and broader
benchmarking across cancer types is needed to establish generality of
the variance-prognosis divergence pattern and of survival supervision as
a remedy. Fourth, DeSurv's convergence guarantee (SI Appendix) applies
...
(25) both predict that supervised compression retains outcome-relevant
...
in other settings where high-dimensional measurements contain both
signal and nuisance variation---including multi-omics integration,
spatial transcriptomics, and electronic health records. Extending DeSurv
...
annotation grow in size, direct survival modeling at cellular resolution
may complement deconvolution-based approaches, though the cohort sizes
...
genes (details in SI Appendix), denoted \(\tilde{W}\). In PDAC, Bayesian
optimization selected ntop\(=270\) top genes per factor; a controlled
K\(\times\alpha\) sensitivity analysis subsequently confirmed that this
gene-focusing parameter, rather than the specific \(\lambda\) or \(\nu\)
values, is the critical hyperparameter enabling adjusted external
validation (see Results and SI Appendix, Tables S1--S3).
...
training (TCGA and CPTAC) and the rest were used for external validation
(Dijk, Moffitt, PACA, Puleo). To harmonize differences in scale across
cohorts, filtered gene expression data was within-subject rank
transformed before model training. More details about the datasets can
...
## Paper git commit: 11cf830d5c14cca1a36e410fda7073890789f038
\end{verbatim}
...
\bibitem[\citeproctext]{ref-bailey2016genomic}
\CSLLeftMargin{14. }%
\CSLRightInline{Bailey P, et al. (2016) Genomic analyses identify
molecular subtypes of pancreatic cancer. \emph{Nature}
531(7592):47--52.}
