...
[@elyada2019cross]; activated (proCAF) and resting (restCAF) fibroblast
programs from the deCAF study [@peng2024determination]; SCISSORS PDAC subtypes [@leary2023s]; tumor,
...
```{r fig-converge, fig.width = 5, fig.height= 3, fig.cap="Convergence of model across initializations. \\label{fig:fig-converge}" , fig.env='figure*', fig.pos='htbp', out.height= "3in", out.width='5in'}
...
# K-sensitivity analysis: effect of gene focusing on iCAF factor validation
...
# K=3 factors are partially but asymmetrically nested within K=7
...
img <- magick::image_read("figures/cv_grid/k3_k7_factor_correlation_heatmap.pdf", density = 300)
cowplot::ggdraw() + cowplot::draw_image(img)
```
...
# Direct path to pooled subtype overlap figure
img <- magick::image_read("figures/km_dichot/subtype_overlap_pooled_logrank_tcgacptac.pdf", density = 300)
cowplot::ggdraw() + cowplot::draw_image(img)
```
...
library(cowplot)
# This target lacks metadata; load directly from store objects
fig_gene_overlap_heatmap_desurv_alpha0_tcgacptac <- readRDS(
  file.path(tar_config_get("store"), "objects", "fig_gene_overlap_heatmap_desurv_alpha0_tcgacptac")
)
cowplot::plot_grid(
...
Standard NMF at elbow-selected $k = 5$ and BO-selected $k = 7$ both substantially improve over NMF at $k = 3$ across all cohorts (Table \ref{tab:tab-cindex-comparison}), confirming that additional unsupervised factors capture prognostically relevant variation missed at the matched rank. At $k = 7$, NMF matches or exceeds DeSurv's per-cohort C-index in most cohorts, demonstrating that unsupervised factorization can approach supervised concordance given enough factors. However, this comes at the cost of a less parsimonious and less interpretable factorization: the $k = 7$ NMF solution shows increased fragmentation (Fig. \ref{fig:fig-nmf-k7-heatmap}), and the K-sensitivity analysis (Tables S1--S3) shows that $k = 7$'s prognostic signal largely overlaps with existing molecular classifiers (PurIST, DeCAF), whereas $k = 3$ DeSurv retains significance after classifier adjustment. DeSurv's advantage thus lies not in maximal concordance but in recovering a compact, interpretable factorization whose prognostic content is independent of known classifiers.

To quantify this directly, Table \ref{tab:tab-hr-adjusted} reports pooled validation hazard ratios (per SD of the linear predictor) for DeSurv at $k = 3$ and standard NMF at BO-selected $k = 7$, both unadjusted and after adjustment for PurIST and DeCAF molecular classifiers. In the unadjusted analysis, both methods show a survival association, though DeSurv's effect size is substantially larger. After adjustment, DeSurv retains a significant and clinically meaningful hazard ratio, whereas NMF's effect attenuates and is no longer significant, confirming that NMF's prognostic content at $k = 7$ is largely redundant with existing classifiers.
...
tar_load(val_latent_desurv_alpha0_tcgacptac)
tar_load(data_val_filtered_tcgacptac)
...
fmt_hr <- function(s) {
  paste0(s$hr, " (", s$ci_lo, "--", s$ci_hi, "), ", fmt_p(s$p))
}
...
kable(tab_df, format = "latex", escape = FALSE, booktabs = TRUE,
      caption = "Pooled external validation hazard ratios (per SD of linear predictor) for DeSurv at $k = 3$ and standard NMF at BO-selected $k = 7$ ($\\alpha = 0$). Adjusted models include PurIST and DeCAF molecular classifiers as covariates, with stratification by dataset. \\label{tab:tab-hr-adjusted}") |>
  kable_styling(latex_options = c("hold_position"), font_size = 9)
```
