...
prepare_data_for_clustering <- function(tops, data, facs, WtX = FALSE) {
  ...
  ## pull out key variables
    ...
    keep_genes <- intersect(rownames(ex), genes)
    Xtemp <- ex[keep_genes, ]
    ...
    Xtemp = t(data$Z_scaled)
    kept_sample_ids = data$scores$sample_id
  }
