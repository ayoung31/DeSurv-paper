...
#' Create gene overlap tables with highlighting
#'
#' Generates \pkg{gt} tables showing overlaps between
#' a model's top genes and user-specified gene lists.
#'
#' @param results A results object containing at least:
#'   - `results$model_save_dir`: directory path
#'   - `results$tops`: a data frame of top-ranked genes
#' @param gene_lists A named list of gene sets. Each element should itself
#'   be a list of character vectors (gene symbols).
#' @param which.lists Character vector of names from `gene_lists` to use.
#'   Default is `"all"`, meaning all available lists.
#' @param color.lists A list of color palettes (character vectors of hex codes)
#'   matching the structure of `gene_lists`.
#' @param text Optional text colors (vector). If `NULL` (default),
#'   text colors are chosen automatically for contrast.
#'
#' @return A named list of \pkg{gt} tables, one per gene list.
#'
#' @examples
#' # tables <- create_table(results, gene_lists, which.lists = "PurIST", color.lists)
#' # tables$PurIST
#'
#' @export
create_table <- function(results,
  ...
  idx <- match(which.lists, names(gene_lists))
  gene_lists <- gene_lists[which.lists]
  
  if (!"tops" %in% names(results)) {
    stop("results does not contain a 'tops' element.")
  }
  tops <- results$tops
  
  # helper for styling gt cells
  builder <- function(x, genes) {
    gt::cells_body(
      columns = !!rlang::sym(x),
      rows = !!rlang::sym(x) %in% genes
    )
  }
  ...
  for (i in seq_along(gene_lists)) {
    ...
    # compute appropriate text color (black/white) if not provided
    text_colors <- if (is.null(text)) {
      c("black", "white")[
        1 + apply(col2rgb(colors), 2, function(x) {
          (sum(x * c(299, 587, 114)) / 1000) < 123
        })
      ]
    } else {
      rep(text, length.out = length(colors))
    }
    
    subtitle_text <- paste(
      paste(colors, names(gene_lists[[i]]), sep = "="),
      collapse = " "
    )
    ...
    for (j in seq_along(colors)) {
      tab1 <- tab1 %>%
        gt::tab_style(
          style = list(
            gt::cell_fill(color = colors[j]),
            gt::cell_text(color = text_colors[j])
          ),
          locations = lapply(
            colnames(tops),
            builder,
            genes = gene_lists[[i]][[j]]
          )
        )
    }
    
    output_tables[[names(gene_lists)[i]]] <- tab1
  }
