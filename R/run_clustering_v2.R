...
run_clustering_v2 <- function(tops, data, gene_lists, color.lists = NULL, type = "bas/clas",
  ...
  if (type == "each") {
    for (i in seq_len(ncol(tops))) {
      data <- run_clustering_internal_v2(tops, data, gene_lists, color.lists, type = i,
                                      facs = facs, dir = dir,
    ...
    # id tumor factors
    data <- run_clustering_internal_v2(tops, data, gene_lists, color.lists, type = "tumor",
                                    facs = facs, dir = dir,
                                    maxKcol = maxKcol, maxKrow = maxKrow,
                                    reps = reps, pFeature = pFeature, pItem = pItem,
                                    seed = seed, clusterAlg = clusterAlg, distance = distance,
                                    save = save, plot = plot, weight=weight, replace=replace)
    # id stromal factors
    data <- run_clustering_internal_v2(tops, data, gene_lists, color.lists, type = "stroma",
                                    facs = facs, dir = dir,
                                    maxKcol = maxKcol, maxKrow = maxKrow,
                                    reps = reps, pFeature = pFeature, pItem = pItem,
                                    seed = seed, clusterAlg = clusterAlg, distance = distance,
    ...
    data <- run_clustering_internal_v2(tops, data, gene_lists, color.lists, type = type,
                                    facs = facs, dir = dir,
                                    maxKcol = maxKcol, maxKrow = maxKrow,
                                    reps = reps, pFeature = pFeature, pItem = pItem,
                                    seed = seed, clusterAlg = clusterAlg, distance = distance,
