# _targets_sims.R - Local Quick version for desktop (20 CPUs, 31GB RAM)
# Reduced settings for faster local testing
library(targets)
library(tarchetypes)
library(crew)
...
SIM_FIGURE_DIR <- file.path("figures", "sim")
SIM_FIGURE_CONFIGS <- list(
  k_hist = list(
...
# ============================================================
# LOCAL QUICK MODE SETTINGS - Reduced for faster testing
# ============================================================
SIM_DATASETS_PER_SCENARIO <- 20L  # Reduced from 100
...
SIM_CV_NFOLDS <- 5L
SIM_CV_NSTARTS <- 4  # Reduced from 30 for local - will run sequentially
SIM_TRAIN_FRACTION <- 0.7
SIM_SPLIT_SEED_OFFSET <- 10000L

# Local desktop: 6 workers with 3 CPUs each = 18 CPUs used
# Main job uses 4GB + 1 CPU, leaving room for 6 workers
SIM_LOCAL_WORKERS <- 6L
SIM_LOCAL_CPUS <- 3L
SIM_LOCAL_MEM_GB <- 4L

SIM_ANALYSIS_CONTROLLER <- crew_controller_slurm(
  name = "sim_analysis",
  workers = SIM_LOCAL_WORKERS,
  seconds_idle = 120,
  options_cluster = crew_options_slurm(
    cpus_per_task = SIM_LOCAL_CPUS,
    memory_gigabytes_required = SIM_LOCAL_MEM_GB,
    time_minutes = 240,
...
# Reduced BO bounds for faster convergence
...
# Reduced BO iterations for local testing
SIM_BO_N_INIT <- 4L   # Reduced from 20
SIM_BO_N_ITER <- 4L   # Reduced from 40
SIM_BO_CANDIDATE_POOL <- 500L  # Reduced from 1500
SIM_BO_EXPLORATION_WEIGHT <- 0.01
SIM_BO_K_LCB_LEVEL <- 0.90

# Only 2 scenarios for quick testing
...
# Reduced analysis specs - only 2 for quick mode
...
# ============================================================
# SIMULATION FUNCTIONS (from main _targets_sims.R)
# ============================================================
...
# ============================================================
# ANALYSIS FUNCTIONS - Modified for local (no nested parallelism)
# ============================================================
...
run_fixed_analysis <- function(dataset_entry, analysis_spec) {
  ...
  # LOCAL: Disable nested parallelism - run sequentially
  fit <- DeSurv::desurv_fit(
    ...
    parallel_init = FALSE,  # DISABLED for local
    ncores_init = 1,        # Sequential
    verbose = FALSE
  )
...
run_bayesopt_analysis <- function(dataset_entry, analysis_spec) {
  ...
  # LOCAL: Disable nested parallelism
  bo_results <- DeSurv::desurv_bayesopt(
    ...
    seed = dataset_entry$metadata$seed,
    parallel_grid = FALSE,  # DISABLED for local
    n_starts = SIM_CV_NSTARTS,
    ncores_grid = 1,        # Sequential
