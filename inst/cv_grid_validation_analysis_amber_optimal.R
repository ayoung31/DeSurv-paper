##############################################################################
# cv_grid_validation_analysis_amber_optimal.R
#
...
# Fixed parameters: ntop=270, lambda=0.349, nu=0.056, 100 initializations.
#
...
for (nm in names(fits)) {
  ...
  # Build focused W matrices for H-score correlation
  tops_new <- get_top_genes(W, 270)
  W_new_f <- W
  for (j in seq_len(ncol(W))) {
    keep_genes <- tops_new$top_genes[, j]
    non_top <- setdiff(rownames(W), keep_genes)
    W_new_f[non_top, j] <- 0
  }
  W_orig_f <- orig_W
  ...
  genes_t <- Reduce(intersect, list(rownames(W_new_f), rownames(W_orig_f), rownames(X_train)))
  H_orig_f <- t(X_train[genes_t, ]) %*% W_orig_f[genes_t, ]
  H_new_f  <- t(X_train[genes_t, ]) %*% W_new_f[genes_t, ]
  cors <- cor(H_orig_f[,1], H_new_f, method="spearman")
  ...
  train_df <- build_train_df(W, X_train, surv_train, all_train_sub, ntop=270)
  val_df   <- build_val_df(W, val_data, ntop=270)
...
cat("\n\n========== H_cor MATRIX (ntop=270, lambda=0.349, nu=0.056) ==========\n")
cat(sprintf("%-6s","K"))
