...
desurv_fit <- run_coxNMF(X=X_train,y=surv_train$time,
                                 delta=surv_train$status,k=4,
                                 alpha=.35,lambda=.1,eta=.9,tol=1e-5,
                                 imaxit=50,ninit=5,maxit=6000,verbose=TRUE)

W_desurv    <- desurv_fit$W        # G x K
beta_desurv <- desurv_fit$beta     # length K (optional but nice)
