...
saveRDS(val_p_adj_matrix, file.path(OUT_RDS, "adj_p_all_matrix.rds"))
saveRDS(val_p_matrix,     file.path(OUT_RDS, "unadj_p_all_matrix.rds"))
saveRDS(hcor_matrix,      file.path(OUT_RDS, "hcor_all_matrix.rds"))
master_df_all <- do.call(rbind, lapply(names(master_rows), function(nm) {
  r <- master_rows[[nm]]
  data.frame(
    nm       = nm,
    k        = r$k,
...
saveRDS(master_df_all, file.path(OUT_RDS, "master_rows_all.rds"))
cat(sprintf("\nOutputs saved to %s/\n", OUT_RDS))
cat("  adj_p_all_matrix.rds\n  unadj_p_all_matrix.rds\n  hcor_all_matrix.rds\n  master_rows_all.rds\n")
